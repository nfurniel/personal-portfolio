...
/*
 * Everything that pulls in three.js lives here, behind the lazy() boundary in
 * ContactFigure. Importing it from the section directly would put the whole
 * three + @react-three/fiber chunk (~900 kB) on the first-paint critical path
 * for a decorative figure that sits at the very bottom of the page.
 */
...
// El GLB va comprimido con EXT_meshopt_compression, así que el loader necesita
// el decodificador. Viene con three, sin CDN de por medio.
const withMeshopt = (loader) => loader.setMeshoptDecoder(MeshoptDecoder)

function Model({ pointer, reduceMotion }) {
  const gltf = useLoader(GLTFLoader, modelUrl, withMeshopt)
...
export default function FigureScene({ active, reduceMotion, onReady }) {
  ...
  return (
    <Canvas
      ...
      // Scrolled past, or tab in the background: stop feeding the GPU. The
      // figure is decorative and the loop is pure cost once it is out of sight.
